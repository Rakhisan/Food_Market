          ...
          {itemCards?.map((cardb, index) => {
            return (
              <div id="res-menu-cardele" key={"card" + index}>
                <div>
                  {cardb?.card?.info?.itemAttribute?.vegClassifier == "VEG" ? (
                  ...
                  <p>
                    {"₹" +
                      (cardb?.card?.info?.price ??
                        cardb?.card?.info?.defaultPrice) /
                        100}
                  </p>
                  <p className="card-head">{cardb?.card?.info?.description}</p>
                </div>
                <div className="menu-img-box">
                  {cardb?.card?.info?.imageId ? (
                    <img
                      className="menu-img"
                      src={IMG_SMALL_URL + cardb?.card?.info?.imageId}
                    />
                  ) : (
                    <div className="menu-img"></div>
                  )}
                  {itemsCart?.items?.filter(
                    (item) => item[0]?.id == cardb?.card?.info?.id
                  ).length == 0 ? (
                    <>
                      <button
                    ...
                    <span className="cart-remove-btn" id="item-add">
                      <button
                        onClick={() => {
                          removeFoodItem(cardb?.card?.info?.id);
                      ...
                      <button
                        onClick={() => {
                          increaseFoodItem(cardb?.card?.info?.id);
                          toast.success("Item added successfully");
                        }}
                      >
                        +
